...
| P2-50b | 메시지 저장 (UIMessage[] 스냅샷) | P2-50a 전략 기반. conversations 테이블에 `ui_messages jsonb` 컬럼 추가(migration + schema.dbml). chat.ts: (1) `toUIMessageStreamResponse({ originalMessages, onFinish })` 패턴 전환 (2) `result.consumeStream()` 클라이언트 끊김 대비 (3) 기존 afterWork → onFinish 내부 통합 (4) History API를 ui_messages 조회로 변경. 전략: `message-persistence-strategy.md` | P2-50a | ⬜   |
| P2-50c | Chat 히스토리 클라이언트 로드 | P2-50b 완료 후. ChatInterface 마운트 시 히스토리 로드 → `useChat({ messages })` 전달 → card-mapper가 tool parts에서 카드 자동 복원. 로딩 상태 UI. 중복 메시지 방지 검증. 세션 쿠키 기반. **대화방 목록/전환은 v0.2** — MVP는 마지막 대화 1개만 이어가기 | P2-50b | ⬜   |
