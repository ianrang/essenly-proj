...
| NEW-40  | 제품 데이터 정합성 복구 + 가격 수집 | **71건 브랜드-링크 불일치 수정 + 200건 전체 가격 수집**. (1) enriched.json 기반 brand 복원 + 고아 제품 제거 → products-recovered.json (2) 올리브영 Global/한국 멀티소스 스크래핑 (브랜드 검증 게이트 추가) — 이미지 + 구매 링크 + 가격(USD→KRW 환율 변환) 수집 (3) 전수 정합성 검증 + 리포트 → products-validated.json 갱신 + DB 적재. NEW-37 스키마 필드 완전 채움 (price_source='real', price_currency='KRW'). 설계: `docs/superpowers/specs/2026-04-14-product-data-recovery-design.md`. 의존: NEW-37 ✅ | ⬜   |
