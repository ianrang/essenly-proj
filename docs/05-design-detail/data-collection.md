...
### 네이버 쇼핑 API — MVP 활용 범위

네이버 쇼핑 API 이용약관 원문이 미확인 상태이므로 (U-5), **MVP에서는 프로그래밍적 API 호출을 사용하지 않는다.** 관리자가 네이버 쇼핑 **웹사이트**를 직접 검색하여 참조하는 것은 일반 사용자 행위와 동일.

| 사용 방식 | MVP 허용? | 근거 |
|----------|---------|------|
| 관리자가 **브라우저에서** 네이버 쇼핑 웹사이트 검색 → 가격/카테고리 참조 → CSV에 수동 기입 | **허용** | 일반 사용자 행위 |
| 관리자가 **API를 코드로 호출**하여 제품 검색 | **약관 확인 후** (U-5 보완 필요) | API 호출 = 약관 적용. 브라우저 검색과 법적으로 다름 |
| API 결과를 자동으로 자체 DB에 대량 적재 | **사용 안 함** | 약관 미확인 |
...
scripts/seed/lib/                      ← 수집 파이프라인 로직 (CLI 전용 + 관리자 앱 공유)
  ...
  │   ├── naver-shopping.ts            ← S2 **MVP 미구현 (U-5: API 약관 미확인. 브라우저 수동 검색으로 대체)**
...
| **U-5** | **네이버 쇼핑 API 이용 범위** | **판정 재수정 (2026-03-24)** | API 약관 위반 시 키 차단 | **재평가 (5팀 리뷰 반영)** | **MVP: 프로그래밍적 API 호출 사용 안 함.** 브라우저 수동 검색만 허용 (일반 사용자 행위). API 코드 호출 = 약관 적용이므로 약관 원문 미확인 상태에서 코드 통합 불가. 약관 확인 후 재평가 |
