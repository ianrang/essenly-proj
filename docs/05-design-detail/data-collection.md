...
|--------|---------|------|--------|------|
| products (전량 수동) | ~20분 | 200 | ~67시간 | **S2 보류로 전량 수동.** 올리브영/시코르 사이트 수동 참조 → 제품명·가격·카테고리 입력 → AI 분류 → 전수 검수 |
...
| **합계** | | | **~141시간** | **1인 풀타임(8h/일) 약 18일** |

> 주의: S2(네이버 쇼핑) 보류로 products 공수가 ~50시간→~67시간 증가. 전체 ~124→~141시간. 코딩(~5-7주)과 데이터 입력(~3.5주)을 병행. M1→M2→M3 단계적 진행으로 부하 분산. S2 재평가로 사용 가능해지면 ~50시간으로 복귀.
