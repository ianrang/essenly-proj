...
|--------|----------|----------------|
| ~~P1-33~~ | ~~개인화 추출 방식~~ | **확정: 동기 tool (extract_user_profile)**. 추출=동기 tool, 결과=메모리 보관(동의 전 DB 저장 안 함). 동의 후 POST /api/profile/onboarding으로 DB 저장 (PRD §4-C). tool 스키마는 P1-31에서 정의 |
