...
4. 세션 수신 → 선택한 경로A(/onboarding) 또는 경로B(/chat)로 이동
```
...
| 액션 | 결과 |
|------|------|
| Hero PathAButton 클릭 (신규) | 인라인 동의 표시 → Continue → 세션 생성 + `/[locale]/onboarding` 이동 |
| Hero PathBButton 클릭 (신규) | 인라인 동의 표시 → Continue → 세션 생성 + `/[locale]/chat` 이동 |
| Hero PathAButton 클릭 (동의 완료) | 바로 `/[locale]/onboarding` 이동 |
| Hero PathBButton 클릭 (동의 완료) | 바로 `/[locale]/chat` 이동 (프로필 없이) |
