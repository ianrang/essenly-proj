...
### 1.6 인라인 동의 (HeroSection 내)

**위치**: `client/features/landing/HeroSection.tsx` (CTA 영역 내 조건부 표시)

- CTA 버튼 클릭 시 Hero 영역 내에서 인라인 동의 확인 UI 표시
- "Continue" 클릭 → `POST /api/auth/anonymous` (세션 생성 + consent 기록 동시) → 선택한 경로로 이동
- "Cancel" 클릭 → 동의 UI 숨김, CTA 버튼 복귀
- 이미 동의(세션 존재)한 경우 → CTA 클릭 시 동의 건너뛰고 바로 이동
...
| Landing SecondaryCTA 버튼 | `path_a_entry` | "Set up my profile" 클릭 |
...
- 동적 컴포넌트(인라인 동의, ReturnVisitBanner, CTA 활성/비활성): `"use client"`
...
app/(user)/[locale]/page.tsx              ← SSG, Composition Root (L-2). 풀 너비 레이아웃.
  └─ LandingPage                          ← features/landing/ (page-level wrapper)
       ...
       │    ├─ 타이틀 + 설명
       │    ├─ PrimaryCTA ("Start chatting")          ← 주력: Chat 직행
       │    └─ SecondaryCTA ("Set up my profile")    ← 보조: 선택적 사전 프로필
       ...
       └─ ReturnVisitBanner (조건부)       ← features/landing/ ("use client")
            ├─ "Welcome back" 메시지
            ├─ ContinueChattingButton       ← primary (채팅 우선)
            └─ ViewProfileButton            ← outline (보조)
```
...
| 상태 | 조건 | UI |
|------|------|-----|
| **신규** | 프로필 없음 + 세션 없음 | 전체 섹션 표시. CTA 활성. 클릭 시 인라인 동의 확인 → 동의 후 이동 |
| **동의 완료** | 프로필 없음 + 세션 있음 | 전체 섹션 표시. CTA 클릭 시 바로 이동 (동의 건너뜀) |
...
2b. 404 → 동의 완료 (세션 있으나 프로필 없음). CTA 클릭 시 바로 이동
2c. 세션 없음 → 신규. CTA 클릭 시 인라인 동의 표시
3. 인라인 동의 "Continue" → POST /api/auth/anonymous (consent + 세션 동시 생성)
4. 세션 수신 → 선택한 CTA에 따라 /chat(주력) 또는 /onboarding(보조)으로 이동
```
...
| 액션 | 결과 |
|------|------|
| Hero PrimaryCTA 클릭 (신규) | 인라인 동의 표시 → Continue → 세션 생성 + `/[locale]/chat` 이동 |
| Hero SecondaryCTA 클릭 (신규) | 인라인 동의 표시 → Continue → 세션 생성 + `/[locale]/onboarding` 이동 |
| Hero PrimaryCTA 클릭 (동의 완료) | 바로 `/[locale]/chat` 이동 (프로필 없이, VP-3 점진적 개인화) |
| Hero SecondaryCTA 클릭 (동의 완료) | 바로 `/[locale]/onboarding` 이동 |
| ContinueChattingButton 클릭 (재방문) | `/[locale]/chat` 이동 (기존 프로필 적용) |
| ViewProfileButton 클릭 (재방문) | `/[locale]/profile` 이동 |
| LanguageSelector 변경 | 대화 언어 Context 업데이트. UI 영어 유지 |
| 인라인 동의 Cancel | 동의 UI 숨김, CTA 버튼 복귀 |
