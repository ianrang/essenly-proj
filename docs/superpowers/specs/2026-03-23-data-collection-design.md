...
| MVP 행위 | 법적 분류 | 등록 필요? |
|---------|---------|----------|
| AI 대화로 제품 정보 제공 | 정보제공 서비스 (부가통신사업) | 통신판매업 불필요 |
| 외부 쇼핑몰 링크 제공 (F2) | 어필리에이트/중개 | 어필리에이트 고지 필요 |
| 시술 정보 제공 | 정보제공 (의료 조언 아님) | 면책 문구 필수 |
| 에센리 키트 직접 판매 (후기) | 통신판매 | 키트 판매 시작 시 등록 필요 |
...
추천·보증 등에 관한 표시·광고 심사지침(공정거래위원회 고시) 제5조:
...
- **skin_type**: 모든 5개 타입에 skincare 40개+ 적합 제품
...
| products | name.ko, price(참조), category, brand(매칭), purchase_links | 번역, skin_types, concerns, description, review_summary | 매장 정가, volume, english_label, images, key_ingredients | ~35% |
...
server/features/pipeline/              ← 재사용 가능 비즈니스 로직
  ├── providers/
...
| **U-2** | 식약처 API 실제 응답 형식 | **중** | API 필드명/형식 불일치 시 코드 수정 | M1 | API 키 발급 → Swagger UI 테스트 |
...
| 커버리지 | 모든 skin_type에 skincare 40+, 모든 concern에 제품 5+ |
