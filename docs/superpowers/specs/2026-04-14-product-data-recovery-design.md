...
`collect-replacement-products.ts` 실행으로 201건 중 71건의 `brand` 필드가 다른 제품의 브랜드로 교체됨. 해당 71건의 `images`, `purchase_links`, `price`도 엉뚱한 제품의 데이터. 고아 제품 1건(Essenly 자체 브랜드, enriched.json에 없음) 존재.
...
| 고아 제품 (enriched.json에 없음, 구조 다름) | 1 | 제거 |
| 가격 있음 (정상) | 2 | 전부 오염 데이터 가격 |
...
products-validated.json (201건, 71건 오염 + 1건 고아)
```

## 2. 목표

- 200건 전체 제품의 brand/name/images/purchase_links/price 정합성 확보
- 올리브영에서 찾을 수 없는 제품은 리스트업 (images/links=null, 가격은 후속 파이프라인)
- NEW-37 스키마 필드 (`price_source`, `price_currency`, `price_source_url` 등) 완전 채움
...
2. 고아 제품(ID `55265963`) 제거
3. 기존 오염된 `images`, `purchase_links`, `price` 초기화 (71건)
4. 정상 129건의 기존 데이터 보존
5. 출력: `products-recovered.json` (200건, 정본 brand/name + 정상 129건 이미지/링크 유지)

### Phase 2: 올리브영 스크래핑 (Playwright)

대상: 200건 전체 (정상 129건도 가격 수집 필요)

#### 스크래핑 소스 우선순위

```
1차: 올리브영 Global (global.oliveyoung.com)
     → 영어 UI, 이미지, 구매 링크, USD 가격
     → USD → KRW 환율 변환 저장
...
| `price` | 할인가 or 정가 (현재 판매가) | `int` (KRW) |
| `price_min` | 할인가 (있을 경우) | `int` (KRW) |
| `price_max` | 정가 | `int` (KRW) |
...
- Global USD 가격 수집 시: `Math.round(usdPrice * exchangeRate)` → KRW 저장
- 환율: 스크립트 실행 시 공개 API에서 1회 조회 또는 고정 환율 상수 사용
- `price_source_url`에 Global URL 기록 → 원본 USD 가격 역추적 가능
...
5. 최종 실패 → images=[], purchase_links=[], price=null
```
...
2. 리포트 생성: `products-recovery-report.json`
   - 매칭 성공/실패 목록
   - 소스별 (Global/한국) 건수
   - 가격 커버리지
...
| `docs/audit/product-recovery-report.md` | 매칭 결과 리포트 |
...
- `price NOT BETWEEN min AND max`: CHECK 미적용 → 세일 수용 ✅

## 6. NEW-35/36 영향

- **NEW-35 (가격 티어)**: KRW 통일 저장으로 기존 임계값(₩23.5k/₩48.5k) 그대로 사용 가능
...
| 19개 브랜드 올리브영 미입점 | ~30건 이미지/링크 부재 | 리스트업 후 사용자 판단. 카드 UI는 이미 null 대응 |
| 환율 변동 | USD→KRW 가격 부정확 | `price_updated_at` 기록, 정기 갱신 가능 |
| G-12 외부 소스 검증 | 법적 리스크 | NEW-27에서 동일 소스 사용 전례. 상업적 사용 아닌 참조 목적 |
