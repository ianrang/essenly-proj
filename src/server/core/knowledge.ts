...
import { getEmbeddingModel, getEmbeddingProviderOptions } from './config';
...
export async function embedQuery(text: string): Promise<number[]> {
  ...
  const { embedding } = await embed({
    model,
    value: text,
    providerOptions: getEmbeddingProviderOptions('RETRIEVAL_QUERY'),
  });
  return embedding;
}
...
export async function embedDocument(text: string): Promise<number[]> {
  ...
  const { embedding } = await embed({
    model,
    value: text,
    providerOptions: getEmbeddingProviderOptions('RETRIEVAL_DOCUMENT'),
  });
  return embedding;
}
